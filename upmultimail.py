...
Dtitle = {'22': 'Triangle三角地', '71': 'SecondHand跳蚤市场', '99': 'Job工作', '167': 'PieBridge鹊桥', '230': 'House租房',
...
Dcolor = {'22': '#D2E9FF', '71': '#F1E1FF', '99': '#E8FFF5', '167': '#FFECF5', '230': '#D1E9E9',
...
def terms(time,str):
    with open('html updated version/h3.html.txt','r') as f3:
        s = f3.read().replace('\n', '')
    return s.format(time,str)                                 # 单个话题的爬取内容

def table(subject,bgcolor,preterms):
    with open('html updated version/h2.html.txt','r') as f4:
        s = f4.read().replace('\n', '')
    return s.format(bgcolor,subject,preterms)                 # 每个话题的邮件版面

def framework(emailaddr,pretable):
    with open('html updated version/h1.html.txt','r') as f5:
        s = f5.read().replace('\n', '')
    return s.format(pretable,emailaddr)                       # 整个html文本
...
def contexts(emailaddr,lt,ls):
    ...
    for x in lt:
        if x in ltitle:
            ...
            tempitems = ''
            for i in range(k):
                tempitems += terms(ls[m+i][0],ls[m+i][3])
            temptable += table(x,Dcolor[ls[m][1]],tempitems)
...
for x in luser:
    ...
    if set(ltitle)&set(ltag) == set():                       # 排除暂时无需发送的邮件
        ...
        msg['From'] = _format_addr('bbs_server_developer <{}>'.format(from_addr))
